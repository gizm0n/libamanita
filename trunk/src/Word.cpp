...
void Word::setSeed(unsigned long n) {
	if(rnd==&::rnd) rnd = new Random(n);
	else rnd->setSeed(n);
}

char *Word::generate(int minlen,int maxlen) {
	size_t l = minlen+(minlen>=maxlen? 0 : rnd->getUint32(maxlen-minlen));
	size_t i = 0,n = 0,r = rnd->getUint32(2),a = 0;
	if(l>len) {
	...
	while(i<l) {
		if(r) {
			str[i++] = ch = c[rnd->getUint32(cl)];
			if(i<l && rnd->getUint32(2)) {
				if(a && strchr(d,ch)) {
					if((ch=='c' || ch=='s') && rnd->getUint32(2)) ch = 'h';
					str[i++] = ch;
				} else if(!strchr(s,ch)) {
					while(ch==(str[i]=c2[n=rnd->getUint32(c2l)]) && strstr(u,str+i-1));
					ch = str[i++];
				}
			...
			str[i++] = ch = v[n=rnd->getUint32(vl)];
			if(i<l && strchr(d,ch) && rnd->getUint32(2)) {
				while(ch==v2[n=rnd->getUint32(v2l)]);
				str[i++] = ch = v2[n];
			}
			r = 1,a = rnd->getUint32(2);
		}
	}
	str[0] -= 32;
	str[i] = '\0';
