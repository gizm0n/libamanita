		...
		switch(c) {
			...
			case '\\':
				switch(c=exp[++i]) {
					...
					case 'X':
						if(ishex(exp[i+1])) {
							q.c = xtoi(exp[++i]),q.f |= REGEX_CHAR;
							if(ishex(exp[i+1])) q.c = (q.c<<4)|xtoi(exp[++i]);
						}
						break;
					default:
...
PRINTF("{\n");
					...
					while(i<len-1 && isdigit(c=exp[++i])) q.min = (q.min*10)+(c-'0');
					if(c==',') while(i<len-1 && isdigit(c=exp[++i])) q.max = (q.max*10)+(c-'0');
					else q.max = q.min;
			...
			if(q.c || q.f) {
				if((b=q.c)) chars[b>>5] |= 1<<(b&31);
					...
					if(q.f&REGEX_WORD) for(b=0; b<=0xff; b++) {
						if(!isword(b)) chars[b>>5] |= 1<<(b&31);
					} else if(q.f&REGEX_DIGIT) for(b=0; b<=0xff; b++) {
						if(!isdigit(b)) chars[b>>5] |= 1<<(b&31);
					} else if(q.f&REGEX_SPACE) for(b=0; b<=0xff; b++) {
						if(!isspace(b)) chars[b>>5] |= 1<<(b&31);
					}
				}
...
//PRINTF("\nREBlock::test(%p,%c,%c,%c,%c,%c);\n",this,brack,c0,c1,c2,code!=0? code[i].c : ' ');
	if(brack==REGEX_SQRBR) {
		...
		if(brack==REGEX_RNDBR) for(recode q; i<len; i++) {
			...
			else if(q.f&REGEX_ANY) return ((re->mod&REGEX_DOTALL) || !isbreak(c1));
			else if(q.f&REGEX_WORD) return (isword(c1)^n);
			else if(q.f&REGEX_DIGIT) return (isdigit(c1)^n);
			else if(q.f&REGEX_SPACE) return (isspace(c1)^n);
			else if(q.f&REGEX_STRING_B) return (c0=='\0');
			else if(q.f&REGEX_STRING_E) return (c1=='\0' || ((q.f&REGEX_LINE_E) && isbreak(c1) && c2=='\0'));
			else if(q.f&REGEX_LINE_BML) return (c0=='\0' || isbreak(c0));
			else if(q.f&REGEX_LINE_EML) return (c1=='\0' || isbreak(c1));
			else if(q.f&REGEX_WORD_B) {
				if(q.f&REGEX_WORD_E)
					return (((c0=='\0' || isspace(c0))^n) || ((c1=='\0' || isspace(c1))^n));
				return ((c0=='\0' || isspace(c0))^n);
			}
			else if(q.f&REGEX_WORD_E) return ((c1=='\0' || isspace(c1))^n);
