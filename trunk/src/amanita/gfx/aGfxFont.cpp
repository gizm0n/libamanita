...
// ::fprintf(stderr,"aGfxFont::aGfxFont(1)\n");
// fflush(stderr);
	...
	name = strdup(file);
//::fprintf(stderr,"aGfxFont::aGfxFont(2.1)\n");
//fflush(stderr);
	if(style!=TTF_GetFontStyle(font)) TTF_SetFontStyle(font,style);
//::fprintf(stderr,"aGfxFont::aGfxFont(2.2)\n");
//fflush(stderr);
...
::fprintf(stderr,"aGfxFont::aGfxFont(%s,size=%d,style=%d,height=%d,ascent=%d,descent=%d,lineskip=%d)\n",
...
//::fprintf(stderr,"aGfxFont::aGfxFont(3)\n");
//fflush(stderr);
	TTF_CloseFont(font);
//::fprintf(stderr,"aGfxFont::aGfxFont(4)\n");
//fflush(stderr);
}


aGfxFont::~aGfxFont() {
::fprintf(stderr,"aGfxFont::~aGfxFont(name=%s)\n",name);
fflush(stderr);
	if(name) { free(name);name = 0; }
	for(int i=0; i<256; i++) {
// ::fprintf(stderr,"aGfxFont::~aGfxFont(i=%d,c=%c)\n",i,(char)i);
// fflush(stderr);
		if(glyphs[i]) SDL_FreeSurface(glyphs[i]);
		glyphs[i] = 0;
	}
// ::fprintf(stderr,"aGfxFont::~aGfxFont(done)\n");
// fflush(stderr);
}

int aGfxFont::stringWidth(const char *str) {
	int w = 0;
...
int aGfxFont::stringWidth(const char *str,int i,int l) {
	int w = 0;
	while(*str && i>0) str++,i--;
	if(i>0) return 0;
...
void aGfxFont::print(int x,int y,const char *str) {
	SDL_Rect r;
	unsigned char c;
...
void aGfxFont::print(int x,int y,int a,const char *str) {
...
void aGfxFont::printf(int x,int y,const char *format, ...) {
	char text[1024];
	va_list args;
...
void aGfxFont::printf(int x,int y,int a,const char *format, ...) {
	char text[1024];
	va_list args;
   va_start(args,format);
	vsprintf(text,format,args);
