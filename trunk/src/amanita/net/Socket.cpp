...
const char *Socket::message_names[] = {0,
	"SM_ERR_RESOLVE_HOST",
	"SM_ERR_OPEN_SOCKET",
	"SM_ERR_CONNECT",
	"SM_ERR_BIND",
...
void Socket::print_packet(const uint8_t *data,size_t l) {
	fputc('[',stderr);
...
Socket::Socket(socket_event_handler seh) : event_handler(seh) {
	host = 0;
	status = 0;
	sock = 0;
	ip = 0;
...
void Socket::resolveConnection(const char *con,uint32_t &ip,uint16_t &port,uint32_t &id,char *nick,int nlen) {
	...
	port = p;
	id = d;
debug_output("Socket::resolveConnection(ip=%x,port=%d,id=%d)\n",ip,port,id);
}
...
uint8_t *Socket::receive(tcp_socket_t s,size_t &l) {
	uint8_t *b = buf;
	int r;
	size_t i = 0;
	do {
...
debug_output("Socket::receive(0,r=%d,cmd=%d,len=%d)\n",
...
debug_output("Socket::receive(1,r=%d,i=%lu)\n",r,(unsigned long)i);
		...
		while(r>0 && l<i) {
			...
			l += r;
debug_output("Socket::receive(3,r=%d,l=%lu)\n",r,(unsigned long)l);
		}
		if(l==i) {
debug_output("Socket::receive(");
...
debug_output(")\n");
		...
		if(r==-1) {
debug_output("Socket::receive(error=%s)\n",getError());
		}
		if(b!=buf) free(b);
	}
...
size_t Socket::send(tcp_socket_t s,uint8_t *d,size_t l) {
	if(d && l>0) {
debug_output("Socket::send(0,s=%p,l=%lu)\n",(void *)((intptr_t)s),(unsigned long)l);
#ifdef SOCKET_HEADER_INCLUDED
		int n;
...
#endif /*SOCKET_HEADER_INCLUDED*/
debug_output("Socket::send(1,l=%lu)\n",(unsigned long)l);
	}
