...
debug_output("Http::postf(host=%s,url=%s)\n",host,url);
	String str;
	va_list args;
	va_start(args,url);
	str.vappendf(url,args);
	va_end(args);
debug_output("Http::postf(2: host=%s,url=%s)\n",host,str.toCharArray());
...
debug_output("Http::request(host=%s,url=%s)\n",host,url);
...
//#endif
			...
			t2 = http_clock();
			{
				...
				header << "\r\n";
debug_output("Http::request(header=\"%s\",len=%lu)\n",header.toCharArray(),(unsigned long)header.length());
				if(data && len) {
debug_output("Http::request(data=\"%s\",len=%lu)\n",data,(unsigned long)len);
...
debug_output("Http::request(response=\"%s\",r=%d)\n",buf,r);
				if(r<=0) error = 6;
				else {
					int i,n;
					char *p1 = strstr(buf,"\r\n\r\n"),*p2,*p3,*pb;
//debug_output("Http::request(p1=\"%s\")\n",p1);
					if(!p1) error = 7;
					else {
						bool chunked;
						p1[2] = '\0',pb = p1+4;
...
debug_output("Http::request(split header: %s)\n",buf);
						...
						{ // Split header
							...
							while(p1) {
								...
								if(p2) {
									...
									if(p3) {
										*p2 = '\0',p2 += 2,*p3 = '\0';
debug_output("Http::request(header: %s = %s)\n",p1,p2);
										response.put(p1,p2);
									}
								}
								p1 = p3;
...
debug_output("Http::request(encoding: %s)\n",p1);
						//chunked = p1? strcmp(p1,"identity")!=0 : true;
						p2 = (char *)response.getString(http_headers[HTTP_CONTENT_LENGTH]);
debug_output("Http::request(content length: %s)\n",p2);
						n = p2? atol(p2) : 0;
						chunked = p2? false : true;

						r -= (int)(pb-buf);
debug_output("Http::request(n=%d,r=%d,chunked=%d)\n",n,r,chunked);
						for(int chunks=1; 1;) {
//debug_output("Http::request(loop: n=%d,r=%d,len=%d)\n",n,r,body.length());
							if(n>0 && r>0) {
//debug_output("Http::request(append_data[1]: n=%d,r=%d,len=%d)\n",n,r,body.length());
								if(n>r) {
...
//debug_output("Http::request(append_data[2]: n=%d,r=%d,len=%d)\n",n,r,body.length());
							}
							if(n==0) {
//debug_output("Http::request(read_length[1]: n=%d,r=%d,len=%d)\n",n,r,body.length());
								if(chunked) {
									if(r<32) {
									...
									if(chunks>1) pb += 2;
									n = a::xtoi(pb);
//debug_output("Http::request(read_length: n=%d,r=%d,len=%d)\n",n,r,body.length());
									if(n==0) break;
									p1 = strstr(pb,"\r\n")+2;
									r -= p1-pb;
									pb = p1;
							...
							if(n>0 && r==0) {
								...
								while(n>0) {
									...
									pb[r] = '\0';
									if(n>2048) {
//debug_output("Http::request(read_data[1]: n=%d,r=%d,len=%d)\n",n,r,body.length());
										body.append((const char *)pb,(size_t)r);
										n -= r;
										r = 0;
//debug_output("Http::request(read_data[2]: n=%d,r=%d,len=%d)\n",n,r,body.length());
									} else break;
								}
								if(r<0) break;
							}
...
debug_output("Http::request(len=%lu),n=%d\n",len,n);
