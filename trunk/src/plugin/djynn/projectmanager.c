...
fflush(djynn.log);
	...
	if(fp!=NULL) {
		char str[1025];
		int c = g_key_file_get_integer(conf,"files","current_page",NULL);
		for(n=0; 1; ++n) {
...
void sess_open(const char *fn) {
	FILE *fp = fopen(fn,"r");
	if(fp!=NULL) {
		int pos,sel;
		char str[1024],*p1,*p2,open[1024] = "";
		GeanyDocument *doc;
		document_close_all();
		while(!feof(fp)) {
...
//fflush(djynn.log);
	...
	if(d==NULL) {
		...
		if(p!=NULL) *p++ = '\0',d->name = strdup(p);
		else d->name = strdup(str);
...
static void popup_clicked(GtkMenuItem *menuitem,gpointer data) {
	djynn_pm_action((int)((intptr_t)data));
}
...
static void prj_read(djynn_project *p,char *data) {
...
//fflush(djynn.log);
		...
		if(f1->type==DJYNN_PM_FILE) {
			if(*path==G_DIR_SEPARATOR) f1->path = strdup(path);
			else {
				sprintf(str,"%s%c%s",p->path,G_DIR_SEPARATOR,path);
				f1->path = strdup(str);
			}
			name = strrchr(path,G_DIR_SEPARATOR);
			f1->name = strdup(name==NULL? path : name+1);
...
static GdkPixbuf *utils_pixbuf_from_stock(const gchar *stock_id) {
	...
	if(icon_set) return gtk_icon_set_render_icon(icon_set,gtk_widget_get_default_style(),
...
static void djynn_pm_ws_delete() {
	int index = 1,n;
	gchar *title = "Delete Workspace";
	int r = 0;
	r = djynn_msgbox_ask(title,"Are you sure you want to delete the entire workspace \"%s\" and all its projects?",djynn.workspace);
	if(r==GTK_RESPONSE_OK) {
		djynn_cfg_open();
		n = djynn_cfg_get_int(djynn.str.workspace,djynn.str.workspace_n);
		if(n==1) djynn_msgbox_warn(title,"There must be at least one workspace!",NULL);
		else {
			char key[257];
...
static void djynn_pm_sess_delete() {
	int index = 1,n;
	gchar *title = "Delete Session";
	int r = 0;
	char key[64],str[257],*p1,*p2;
	...
	r = djynn_msgbox_ask(title,"Are you sure you want to delete the session \"%s\"?",str);
	if(r==GTK_RESPONSE_OK) {
		n = djynn_cfg_get_int(djynn.str.session,djynn.str.session_n);
		if(n==1) djynn_msgbox_warn(title,"There must be at least one session!",NULL);
		else {
//fprintf(djynn.log,"djynn_pm_ws_delete1(index=%d,workspace=%s)\n",index,djynn.workspace);
//fflush(djynn.log);
...
static void djynn_pm_delete(djynn_project_file *f) {
	if(f==NULL) return;
		...
		if(f->type==DJYNN_PM_PROJECT) {
			...
			title = "Delete Folder";
			message = "Are you sure you want to remove this folder \"%s\" and all its files and subfolders from the project?";
		} else {
			title = "Delete File";
			message = "Are you sure you want to remove this file \"%s\" from the project?";
		}
...
static void item_activate_cb(GtkMenuItem *menuitem, gpointer gdata) {
	djynn_pm_action((int)((intptr_t)gdata));
}
...
//fflush(djynn.log);
			...
			char *fn = strrchr(doc->file_name,G_DIR_SEPARATOR);
			if(fn!=NULL) ++fn;
...
//	GeanyDocument *doc;
	...
	gtk_box_pack_start(GTK_BOX(prj_vbox),workspace_list,FALSE,TRUE,0);
	...
	gtk_box_pack_start(GTK_BOX(prj_vbox),sess_list,FALSE,TRUE,0);
