...
static void list_changed_callback(GtkWidget *widget,gpointer data) {
	...
	weh(l,LIST_CHANGED,0,0,0);
}
#endif


Object_Inheritance(List,Widget)

List::List(widget_event_handler weh) : Widget(weh,WIDGET_LISTVIEW) {
	columns = 0;
	column_widths = 0;
	ncolumns = 0;
	item = 0;
...
void List::create(Window *wnd,uint32_t st) {
	int i;
#ifdef USE_GTK
...
void List::setColumns(const char *cols,const int widths[],char sep) {
...
void List::addRow(const char *cols,char sep) {
	if(cols) {
		if(component) {
			char *p1,*p2,*c = strdup(cols);
			int n;
...
const char *List::getColumn(int n) {
