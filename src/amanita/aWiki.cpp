...
aWikiData::aWikiData(aWiki &w) : ref(0),tag_handlers(),link_handlers() {
	wiki = &w;
	target = 0;
	base_url = 0;
	primary_smileys = 0;
...
aWikiData::~aWikiData() {
	...
	clearSmileys();
	for(iter=tag_handlers.iterate(); (th=(aWikiTagHandler *)iter.next());) free(th);
	for(iter=link_handlers.iterate(); (lh=(aWikiLinkHandler *)iter.next());) free(lh);
}

void aWikiData::setTagHandlers(const aWikiTagHandler *th) {
	aWikiTagHandler *th1;
	for(int i=0; th[i].tag; ++i) {
		th1 = (aWikiTagHandler *)malloc(sizeof(aWikiTagHandler));
		memcpy(th1,&th[i],sizeof(aWikiTagHandler));
//debug_output("setTagHandlers(i: %d, tag: %s)\n",i,th[i].tag);
		tag_handlers.put(th[i].tag,(void *)th1);
	}
}

aWikiTagHandler *aWikiData::getTagHandler(const char *tag) {
	return (aWikiTagHandler *)tag_handlers.get(tag);
}

void aWikiData::setLinkHandlers(const aWikiLinkHandler *lh) {
	aWikiLinkHandler *lh1;
	for(int i=0; lh[i].nmsp; ++i) {
		lh1 = (aWikiLinkHandler *)malloc(sizeof(aWikiLinkHandler));
		memcpy(lh1,&lh[i],sizeof(aWikiLinkHandler));
		link_handlers.put(lh[i].nmsp,(void *)lh1);
	}
}

aWikiLinkHandler *aWikiData::getLinkHandler(const char *nmsp) {
	return (aWikiLinkHandler *)link_handlers.get(nmsp);
}

void aWikiData::clearSmileys() {
	int i;
	if(primary_smileys) {
...
aWikiParams::aWikiParams(aWiki &w,aWikiData &d,aString &t) : wiki(&w),data(&d),text(&t) {
...
void aWikiLink::parse(aWikiData &data,char *text) {
	char *p = text,*p1,*p2;
	...
	if((p1=strchr(p,'('))) {
		*p1++ = '\0';
		...
		parenthesis = aString::toLower(getName(p1));
	}
	if(!title) title = strdup(p);
	name = getName(p);

//debug_output("aWikiLink::parse(name: %s, nmsp: %s, title: %s)\n",name,nmsp,title);
...
void aWikiLink::write(aWiki &w,const char *url) {
	aWiki w1(w);
...
static const int file_types[] = {
	aWIKI_IMAGE,aWIKI_IMAGE,aWIKI_IMAGE,
	aWIKI_DOCUMENT,aWIKI_DOCUMENT,aWIKI_DOCUMENT,aWIKI_DOCUMENT,
	aWIKI_ARCHIVE,
};
...
aWikiFile::aWikiFile(aWikiLink &l,const char *dir[]) {
	...
	image_width = 0;
	image_height = 0;
//debug_output("aWikiFile::aWikiFile(name: %s)\n",link->name);
	if((p=strrchr(link->name,'.'))) {
		ext = aString::toLower(strdup(&p[1]));
		n = aVector::find(file_extensions,ext);
		if(n!=-1) {
			type = file_types[n];
...
//debug_output("aWikiFile::aWikiFile(path: %s, type: %d, n: %d)\n",path,type,n);
			if(type==aWIKI_IMAGE) { // Read image width and height.
				...
				if(fp) {
					if(n==FILE_PNG) {
						...
						char s[11];
						fread(s,11,1,fp);
//debug_output("aWikiFile::aWikiFile(jpg)\n");
						if(!strncmp(s,"\xFF\xD8\xFF\xE0",4) && !strncmp(&s[6],"JFIF\0",5)) {
							unsigned char *s1 = (unsigned char *)s;
							uint16_t bl = swap_be_16(*(uint16_t *)&s[4])-7;
							while(1) {
//debug_output("aWikiFile::aWikiFile(bl: %d)\n",(int)bl);
								if(feof(fp)) break;
								fseek(fp,bl,SEEK_CUR);
								fread(s,4,1,fp);
//debug_output("aWikiFile::aWikiFile(s: %04hx)\n",swap_be_16(*(uint16_t *)s));
								if(*s1!=0xff) break;
//debug_output("aWikiFile::aWikiFile()\n");
								if(s1[1]==0xc0) {
									fread(s,5,1,fp);
...
//debug_output("aWikiFile::aWikiFile(width: %d, height: %d)\n",image_width,image_height);
	...
	if(type==aWIKI_IMAGE) {
		...
		if(link->title && *link->title) {
			...
			while(p) {
				if((p1=strchr(p,'|'))) *p1++ = '\0';
				if(*p=='f' && !strcmp(p,"frame")) style |= aWIKI_IMAGE_FRAME;
				else if(*p=='t' && !strcmp(p,"thumb")) style |= aWIKI_IMAGE_THUMB|aWIKI_ALIGN_RIGHT;
				else if(*p=='l' && !strcmp(p,"left")) style |= aWIKI_ALIGN_LEFT;
				else if(*p=='c' && !strcmp(p,"center")) style |= aWIKI_ALIGN_CENTER;
				else if(*p=='r' && !strcmp(p,"right")) style |= aWIKI_ALIGN_RIGHT;
					...
					*p2 = '\0',p2 = strchr(p,'x');
					...
					if(text) free(text);
					text = strdup(p);
debug_output("aWikiFile::aWikiFile(text: %s)\n",text);
				}
				p = p1;
			}
			free(str);
		...
		} else if((style&aWIKI_IMAGE_THUMB) || upright>0.0) {
...
void aWikiFile::write(aWiki &w,const char *url[]) {
	switch(type) {
		case aWIKI_IMAGE:
		{
			if((style&aWIKI_ALIGN_CENTER)) w << "<div class=\"wiki_img_center\">";
			if((style&aWIKI_IMAGE_FRAME) || (style&aWIKI_IMAGE_THUMB)) {
				w << "<div class=\"wiki_thumb";
				if(!(style&aWIKI_ALIGN_CENTER)) {
					if((style&aWIKI_ALIGN_LEFT)) w << " wiki_thumb_left";
					else if((style&aWIKI_ALIGN_RIGHT)) w << " wiki_thumb_right";
				}
				if(width) w << "\" style=\"width:" << width << "px;";
				w << "\">";
			}
			if(href) w << "<a href=\"" << url[aWIKI_IMAGE] << href << "\" class=\"wiki_img_link\">";
			w << "<img src=\"" << url[aWIKI_IMAGE] << name << "\" alt=\"" << alt << "\" class=\"wiki_img\"";
			if(width) w << " width=\"" << width << '"';
			if(height) w << " height=\"" << height << '"';
			w << " />";
			if(href) w << "</a>";
			if(((style&aWIKI_IMAGE_FRAME) || (style&aWIKI_IMAGE_THUMB)) && text) {
debug_output("aWikiFile::write(text: %s)\n",text);
				w << "<div>";
				w.formatInline(text);
				w << "</div></div>";
			}
			if((style&aWIKI_ALIGN_CENTER)) w << "</div>";
			break;
		}
		case aWIKI_DOCUMENT:
			w << "<a href=\"" << url[aWIKI_DOCUMENT] << name << "\">" << link->title << "</a>";
			if(href) w << "<a href=\"" << url[aWIKI_ARTICLE] << link->link << "\" class=\"wiki_doc\"><div></div></a>";
			break;
		case aWIKI_ARCHIVE:
			w << "<a href=\"" << url[aWIKI_ARCHIVE] << name << "\">" << link->title << "</a>";
			if(href) w << "<a href=\"" << url[aWIKI_ARTICLE] << link->link << "\" class=\"wiki_arc\"><div></div></a>";
			break;
	}
}


static void wiki_char_style(aWikiParams &p) {
...
static void wiki_link(aWikiParams &p) {
	char s[p.length+1];
...
static void wiki_anchor(aWikiParams &p) {
	aString anchor;
...
static void bb_tag_html(aWikiParams &p) {
	aString param;
	if(p.param_length) {
...
static void bb_tag_url(aWikiParams &p) {
	aWiki ref(*p.wiki);
	aString href;
	if(p.param_length) {
...
static void bb_tag_img(aWikiParams &p) {
	*p.wiki << "<img src=\"";
	p.wiki->append(*p.text,(long)p.offset,(long)p.length);
	*p.wiki << "\" alt=\"\" />";
}

static void bb_tag_quote(aWikiParams &p) {
	// TODO
	p.wiki->formatInline(*p.text,false,p.offset,p.length);
}

static void bb_tag_code(aWikiParams &p) {
	*p.wiki << "<pre>";
	p.wiki->formatInline(*p.text,false,p.offset,p.length);
	*p.wiki << "</pre>";
}

static void bb_tag_size(aWikiParams &p) {
	if(p.param_length) {
...
static void bb_tag_color(aWikiParams &p) {
	if(p.param_length) {
...
static void bb_tag_background(aWikiParams &p) {
	if(p.param_length) {
...
aWiki::aWiki(size_t c) : aString(c) {
	flags = 0;
	owner = 0;
	data = new aWikiData(*this);
}

aWiki::aWiki(aWiki &w,size_t c) : aString(c) {
	flags = w.flags;
...
void aWiki::setTarget(const char *target) {
	if(data->target) ::free(data->target);
	data->target = strdup(target);
}

void aWiki::setBaseURL(const char *url) {
	if(data->base_url) ::free(data->base_url);
	data->base_url = strdup(url);
}

void aWiki::format(const char *text,uint32_t f) {
	clear();
	*this << text;
	format(f);
}
...
//debug_output("w1: \"%s\"\n",w1.toCharArray());
	...
	w2.storeRefs(*this);
	if((flags&aWIKI_SMILEYS) && data->primary_smileys)
		w2.replace((const char **)data->primary_smileys);
//debug_output("w1: \"%s\"\n",w1.toCharArray());
	if(flags&aWIKI_SECTIONS) {
...
//debug_output("header[%d]: \"%s\"\n",i,w2.toCharArray());
	...
	if((flags&aWIKI_SMILEYS) && data->secondary_smileys)
...
void aWiki::formatSections(aString &text) {
	char ind[65];
	aVector lines;
	lines.split(text,"\n");
	*ind = '\0';
...
size_t aWiki::formatSections(aVector &lines,size_t ln,int lvl,char *ind,int f) {
...
size_t aWiki::formatPre(aVector &lines,size_t ln) {
	size_t n = lines.size();
	char *line;
	aString text;
	for(; ln<n; ++ln) {
...
size_t aWiki::formatTable(aVector &lines,size_t ln,int lvl,char *ind) {
	size_t n = lines.size();
...
void aWiki::formatInline(const char *text) {
	aString s(text);
	aWiki w(*this);
	w.formatInline(s);
	injectRefs(w);
...
void aWiki::formatInline(aString &text,bool pg,long o,long l) {
	size_t i;
	char c;
	aWikiParams p(*this,*data,text);
...
//debug_putc(c);
		if(c=='\'') { // <-- Character (inline) formatting – applies anywhere.
			...
			if(flags&aWIKI_TAGS) matchWikiTag(p);
//echo '<p>wiki_tag{func='.$func.',tag='.$tag.',param='.$param.',text='.substr($text,$p,$pn).",p=$p,pn=$pn,p1=$p1,p2=$p2,p3=$p3}</p>";
			...
			if(c=='[') {
				if((flags&aWIKI_LINKS))
					matchWikiLink(p);
			} else if(c=='{') {
				matchURL(p);
			} else if((flags&aWIKI_BBCODE)) {
				matchBBTag(p);
...
void aWiki::addRef(const char *ref) {
	data->ref << ref;
	*this << "{^ref:" << data->ref.size() << '}';
}


void aWiki::storeRefs(aString &text) {
	size_t p1,p2,p3;
	aString r;
	...
	if(flags&aWIKI_LINKS) replace(protocols);
//debug_output("storeRefs(%s)\n",toCharArray());
}

void aWiki::injectRefs(aString &text) {
...
void aWiki::matchTemplate(aWikiParams &p) {
...
void aWiki::matchURL(aWikiParams &p) {
...
void aWiki::matchWikiTag(aWikiParams &p) {
	...
	if(!th) html = aVector::find(html_tags,p.tag);
...
//debug_output("matchWikiTag(i: %ld, j: %ld[%c], k: %ld, c: %c)\n",i,j,p.text->charAt(j),k,c);
		if(k!=-1) {
			if(html!=-1) {
				...
				p.length = 0;
				p.end = k+1;
				if((th->style&aWIKI_TAG_SIMPLE) || p.text->charAt(k-1)=='/') --k; // Simple tag <tag />.
				else {
					char tag1[l+2],tag2[l+4];
					sprintf(tag1,"<%s",p.tag);
					sprintf(tag2,"</%s>",p.tag);
					if((th->style&aWIKI_TAG_NESTED))
...
void aWiki::matchWikiLink(aWikiParams &p) {
	long n = p.text->matchNestedTags("[[","]]",p.offset);
