...
int Flood::fill(int *tbl,int w,int h,int x,int y,int t,int rx,int ry) {
	...
	while((p=pop())>=0) {
		...
		if(wrap&FLOOD_VWRAP) while(y1>=-h && (ry==0 || abs(y1-y)<ry) && table[x2+y2*width]==from) y1--,y2 = y1<0? y1+h : (y1>=h? y1-h : y1);
		...
		while(true) {
			...
			else if(((wrap&FLOOD_VWRAP) && y1>=(height<<1)) || (!(wrap&FLOOD_VWRAP) && y1==height)) break;
		}
		lSeek = push(x1-(y1&1),y1,lSeek),rSeek = push(x1+1-(y1&1),y1,rSeek);
	}
...
bool Flood::push(int x,int y,bool b) {
	if(((wrap&FLOOD_HWRAP) && (x<-width || x>=(width<<1))) || (!(wrap&FLOOD_HWRAP) && (x<0 || x>=width))) return b;
	if(((wrap&FLOOD_VWRAP) && (y<-height || y>=(height<<1))) || (!(wrap&FLOOD_VWRAP) && (y<0 || y>=height))) return b;
