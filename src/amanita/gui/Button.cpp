...
Button::Button(int id,int x,int y,int w,int h) : Widget(x,y,w,h) {
	setID(id);
	bt = (button){
		{ 0,0 },
...
Button::Button(Button &b) : Widget(b),bt(b.bt) {
	bt.font = b.bt.font;
	bt.text = 0;
...
Button::~Button() {
printf("%saButton::~Button()\n",indent);
	if(bt.text) { free(bt.text);bt.text = 0; }
}

void Button::setDefaultSettings(Image *img,Font *f,uint32_t data[36]) {
	_s = (_settings){
		img,f,
		{{
