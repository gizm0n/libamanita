...
hash_t crc32(const char *s,bool c) {
...
uint64_t xtoi(const char *str) {
	uint64_t n = 0;
	if(str) for(int i=0; ishex(*str) && i<16; ++i) n = (n<<4)|xtoi(*str),++str;
	return n;
}

char *itox(char *h,uint64_t i,bool upper) {
	int u = 0;
...
int stricmp(const char *str1,const char *str2) {
	...
	for(; c1 && c2; c1=tolower(*str1++),c2=tolower(*str2++))
...
int strnicmp(const char *str1,const char *str2,size_t n) {
	char c1 = tolower(*str1++),c2 = tolower(*str2++);
	for(; n && c1 && c2; c1=tolower(*str1++),c2=tolower(*str2++),--n)
		if(c1!=c2) return c1<c2? -1 : 1;
...
char *stristr(char *str1,const char *str2) {
	...
	for(; (c1=tolower(*str1)); str1++)
		if(c1==c2 && strnicmp(str1,str2,l)) return str1;
	return 0;
}

int tokens(char *str,const char *delim,bool cins) {
...
char **split(char **list,char *str,const char *delim,bool cins) {
	size_t l = strlen(delim),n = 0;
	if(cins) for(; str && *str; ++n) {
		list[n] = str;
		if((str=stristr(str,delim))) *str = '\0',str += l;
	} else for(; str && *str; ++n) {
		list[n] = str;
...
void reverse(char *str,long o,long l) {
	if(str && *str) {
		long n = strlen(str);
		if(o<0) o = n+o;
		if(l<=0) l = n-o+l;
...
size_t trim(char *str,const char *s) {
	if(!str || !*str) return 0;
	size_t l = 0;
	char *p = str;
	if(!s) s = String::whitespace;
...
void printUTF8(char *d,const char *s,size_t o,size_t l) {
	char c;
	size_t i,n;
...
bool ishtmlent(unsigned char c) { return HTMLentities[c].name!=0; }



Object_Inheritance(String,Object)


const char *String::blank = "";
const char *String::endl = STRING_ENDL;
const char *String::whitespace = STRING_WHITESPACE;
...
size_t String::insert(long n,char c) {
	if(c!='\0') {
		if(len==cap) resize(0);
		if(n==(long)len) {
			str[len] = c;
...
size_t String::insert(long n,const char *s,size_t l) {
	if(s) {
		if(!l) l = strlen(s);
		resize(l);
		if(n==(long)len) {
...
size_t String::insert(long n,const char *s,long o,long l) {
	if(s && *s) {
		if(o<0 || l<=0) {
...
#if __WORDSIZE < 64
size_t String::inserti32(long n,int32_t i) {
...
size_t String::insertu32(long n,uint32_t i) {
...
size_t String::inserti64(long n,int64_t i) {
	if(i>=0 && i<=9) return insert(n,(char)('0'+i));
...
size_t String::insertu64(long n,uint64_t i) {
	if(i<=9) return insert(n,(char)('0'+i));
	char s[21],*p = s+20;
	*p-- = '\0';
	for(; i; i/=10) *p-- = '0'+(i%10);
...
size_t String::insert(long n,double f,int d,char c) {
...
size_t String::insertHex(long n,uint64_t i,bool upper) {
	const char *s = upper? upper_hex : lower_hex;
...
size_t String::insertBase(long n,int64_t i,int base) {
...
size_t String::insertBase(long n,uint64_t i,int base) {
	if(i==0) return insert(n,'0');
	char s[22],*p = s+21,b;
	*p-- = '\0';
	for(; i; i/=base) b = i%base,*p-- = b<=9? '0'+b : 'A'+b-10;
...
size_t String::appendf(const char *f, ...) {
	va_list list;
	va_start(list,f);
...
size_t String::vappendf(const char *f,va_list list) {
	if(!f || !*f) return 0;
	int c;
	...
	size_t l = len;
	while(1) {
//debug_output("String::vappendf(f=%s)\n",f);
		while((c=*f++) && c!='%') {
			if(len==cap) resize(0);
			str[len++] = c;
		}
...
size_t String::appendUntil(const char **s,const char *end,const char *trim,bool uesc) {
	if(!s || !*s || !**s) return 0;
	if(!trim) trim = end;
	size_t l = len;
	if(!end || !*end) {
	...
	unsigned long l2 = 0;
//debug_output("String::appendUntil: ");
	while((c=*s2++)!='\0') {
//debug_putc(c);
		if(uesc) {
			if(c=='/') {
...
size_t String::appendUntil(FILE *fp,const char *end,const char *trim,bool uesc) {
	if(!fp) return 0;
	if(!trim) trim = end;
	if(!end) {
		fpos_t pos;
...
size_t String::repeat(char c,size_t n) {
	resize(n);
...
size_t String::repeat(const char *s,size_t l,size_t n) {
	if(s && n>0) {
		if(!l) l = strlen(s);
		resize(l*n);
...
size_t String::includef(const char *format, ...) {
	char buf[128];
	va_list args;
...
size_t String::include(const char *fn) {
	size_t l = 0;
	FILE *fp = fopen(fn,"rb");
	if(fp) {
		l = include(fp);
...
size_t String::print(FILE *fp) const {
	if(fp) return fwrite(str,len,1,fp);
	return 0;
}

size_t String::println(FILE *fp) const {
	size_t n = 0;
	if(fp) {
...
long String::find(char c,long o,long l) const {
	if(str && *str && c!='\0') {
		if(o<0) o = (long)len+o;
...
long String::find(const char *s,long o,long l,long sl) const {
	if(str && *str && s && *s) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)len-o+l;
...
static const char *escape_chars = "0      abtnvfr";
*/
long String::findChar(const char *s,long o,long l) const {
	if(str && *str && s && *s) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)len-o+l;
		if(o>=0 && l>0 && o+l<=(long)len)
...
bool String::equals(const char *s,long o,long l) const {
	if(str && *str && s && *s) {
		if(o<0) o = (long)len+o;
...
int String::compare(const char *s,long o,long l) const {
	if(str && *str && s && *s) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)strlen(s)+l;
		if(o>=0 && l>0 && o+l<=(long)len)
...
long String::matchNestedTags(const char *tag1,const char *tag2,long o,long l,const char *c1,const char *c2) const {
...
long String::matchQuotes(long o,long l) const {
	if(o<0) o = (long)len+o;
	if(l<=0) l = (long)len-o+l;
	char q = str[o];
...
long String::matchToken(const char *delim,long o,long l,int f) const {
	long i,j = -1,n,dl = strlen(delim);
	if(o<0) o = (long)len+o;
	if(o>=0 && (f&TOKEN_LTRIM)) o = right(o);
	if(l<=0) l = (long)len-o+l;
	if(o>=0 && l>0 && o+l<=(long)len) {
		for(i=o,n=o+l; i<n; ++i)
			if(str[i]=='\\' && (f&TOKEN_ESCAPE)) ++i;
			else if((f&TOKEN_DELIM_STR)? strncmp(delim,&str[i],dl)==0 : strchr(delim,str[i])!=NULL) {
				j = i;
				if((f&TOKEN_DELIM_STR) || !isspace(str[i])) break;
			} else if(j>=0) break;
		if(f&TOKEN_RTRIM) j = left(j);
	}
	return j;
}

long String::matchValue(int lang,long o,long l,int f) const {
	if(o<0) o = (long)len+o;
	if(l<=0) l = (long)len-o+l;
	long i = o,n;
	char c,q = str[o];
	...
	if(o>=0 && l>0 && o+l<=(long)len && lang>=0 && lang<LANG_LANGS) {
...
long String::skipComment(int lang,long o,long l) const {
	long i;
	if(o<0) o = (long)len+o;
	if(l<=0) l = (long)len-o+l;
	if(o>=0 && l>0 && o+l<=(long)len && lang>=0 && lang<LANG_LANGS) {
...
long String::skipComments(int lang,long o,long l) const {
	long i,n;
	if(o<0) o = (long)len+o;
	if(l<=0) l = (long)len-o+l;
	if(o>=0 && l>0 && o+l<=(long)len && lang>=0 && lang<LANG_LANGS) {
...
size_t String::replace(const char *s,const char *r) {
	if(!s || !*s || !r) return 0;
	size_t n,l = len;
	n = count(s);
	if(n>0) {
...
size_t String::replace(const char *s, ...) {
	size_t n,c = 32,l = len;
	va_list vl;
	const char **arr = (const char **)malloc(c*sizeof(const char *)),*vp = s;
	va_start(vl,s);
//debug_output("String::replace(arr[%d]: \"%s\")\n",0,s);
...
size_t String::replace(const char **arr) {
	size_t i,n,l = len;
	for(n=0; arr[n]; ++n);
	if(n>1) {
		char *p = str;
		size_t p1;
		int ln[n],r;
//debug_output("String::replace(n: %d,cap: %d)\n",(int)n,(int)cap);
...
size_t String::stripComments(int lang,long o,long l) {
	long i,m = 0,n;
	if(o<0) o = (long)len+o;
	if(l<=0) l = (long)len-o+l;
...
//debug_output("String::stripComments(m=%d)\n",(int)m);
	return (size_t)m;
}

/*size_t String::stripCComments() {
	char *p1,*p2,*p3;
	size_t n;
...
/*size_t String::stripHTMLComments() {
	char *p1,*p2,*p3;
	size_t n;
	for(p1=str,p2=p1,n=0; *p1!='\0'; ++p1) {
...
size_t String::substr(String &s,long o,long l) const {
	if(str && *str) {
		if(o<0) o = len+o;
		if(l<=0) l = len-o+l;
...
size_t String::substr(char *s,long o,long l) const {
	if(str && *str) {
		if(o<0) o = len+o;
		if(l<=0) l = len-o+l;
		memcpy(s,&str[o],l);
...
void String::newline(const char *nl) {
...
void String::escape(long o,long l,const char *s,int f) {
	if(str && len) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)len-o+l;
		if(o>=0 && l>0 && o+l<=(long)len) {
			...
			for(i=o,m=0,n=o+l; i<n; ++i) // Count number of characters to escape
				if((c=(uint32_t)str[i]) && strchr(escape_sequences,c) &&
							(!isquote(c) || (f&ESCAPE_QUOTE))) ++m; // Escape sequence
...
void String::unescape(long o,long l,int f) {
	if(str && len) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)len-o+l;
		if(o>=0 && l>0 && o+l<=(long)len) {
				...
				for(n=0; i+n<=(long)len; ++i)
					if(i<o+l && (c=str[i+n])=='\\') {
						...
						if(c!=' ' && (p=strchr(escape_chars,c)) &&
							(!isquote(c) || (f&ESCAPE_QUOTE))) str[i] = (char)(p-escape_chars);
						else if(c=='x' && (f&ESCAPE_HEX)) str[i] = (xtoi(str[i+n+1])<<4)|xtoi(str[i+n+2]),n += 2;
						else if((c=='u' || c=='U') && (f&ESCAPE_UNICODE)) {
							for(j=0,m=(c=='u'? 4 : 8),u=0; j<m; ++j) u = (u<<4)|xtoi(str[i+n+1]),++n;
							if(u>0x7f) {
...
void String::quote(const char c) {
	if(c && str && len) {
		if(len+2>=cap) resize(0);
		*str = c;
...
void String::encodeURL() {
	if(str && len) {
		size_t i,n = 0;
		for(i=0; i<len; i++) if(!isurl(str[i]) && str[i]!=' ') n += 2;
		if(n) {
...
c = (unsigned char)p;\
if((c=='\'' || c=='"') && !(f&HTML_QUOTE)) e = 0;\
else if((c=='&') && !(f&HTML_AMP)) e = 0;\
else if((c=='<' || c=='>') && !(f&HTML_LTGT)) e = 0;\
else {\
	e = &HTMLentities[c];\
	if(e->len) {\
		if(*e->name=='#' && !(f&HTML_CODES)) e = 0;\
	}\
}

void String::encodeHTML(int f) {
	if(str && len) {
		size_t i,n = 0;
		unsigned char c;
		const entity *e;
...
void String::decodeUTF8() {
}

void String::encodeBase64() {
...
void String::reverse(long o,long l) {
	if(str && *str) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)len-o+l;
		if(o>=0 && l>0 && o+l<=(long)len) {
...
void String::trim(long &o,long &l,const char *s) const {
	if(str && *str) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)len-o+l;
		if(!s) s = whitespace;
		if(o>=0 && l>0 && o+l<=(long)len) {
//debug_output("String::trim(o: %d, l=%d, s: \"%s\", f: %x, len: %d)\n",(int)o,(int)l,s,f,(int)len);
			while(strchr(s,str[o]) && l>0) ++o,--l;
			while(strchr(s,str[o+l]) && l>0) --l;
		}
	}
}

void String::expand(long &o,long &l,const char *s) const {
	if(str && *str) {
		if(o<0) o = (long)len+o;
		if(l<=0) l = (long)len-o+l;
		if(!s) s = whitespace;
		if(o>=0 && l>0 && o+l<=(long)len) {
//debug_output("String::trim(o: %d, l=%d, s: \"%s\", f: %x, len: %d)\n",(int)o,(int)l,s,f,(int)len);
			while(strchr(s,str[o-1]) && o>0) --o,++l;
			while(strchr(s,str[o+l]) && o+l<(long)len) ++l;
		}
	}
}

long String::left(long o,const char *s) const {
	if(str && *str) {
		if(o<0) o = (long)len+o;
		if(!s) s = whitespace;
		if(o>=0 && o<(long)len) 
...
long String::right(long o,const char *s) const {
	if(str && *str) {
		if(o<0) o = (long)len+o;
		if(!s) s = whitespace;
		if(o>=0 && o<(long)len) 
			while(strchr(s,str[o]) && o<(long)len) ++o;
	}
	return o;
}
...
void String::resize(size_t l) {
	...
	str[cap] = '\0';
//debug_output("String::resize(len=%d,l=%d,cap=%d)\n",len,l,cap);
}

long String::move(long o,long l) {
	if(o<0) o = (long)len+o;
...
void String::setCapacity(size_t n) {
	...
	str[cap] = '\0';
//debug_output("String::resize(len=%d,n=%d,cap=%d)\n",len,n,cap);
}

char String::charAt(long i) const {
	if(i<0) i = (long)len+i;
...
long String::toInt() const {
	char *p = str;
	while(isspace(*p)) ++p;
	if(*p=='0' && *++p=='x') return xtoi(++p);
	return atol(p);
}

size_t String::toIntArray(long *n,char c) const {
