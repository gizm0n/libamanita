...
aGfxButton::_settings aGfxButton::_s = { 0,0,{} };

aGfxButton::aGfxButton(int id,int x,int y,int w,int h) : aGfxComponent(x,y,w,h) {
	setID(id);
	bt = (button){
		{ 0,0 },
...
aGfxButton::aGfxButton(aGfxButton &b) : aGfxComponent(b),bt(b.bt) {
	bt.font = b.bt.font;
	bt.text = 0;
	if(b.bt.text) setText(b.bt.text);
	setMouseListener(this);
}
aGfxButton::~aGfxButton() {
printf("%saButton::~aGfxButton()\n",indent);
	if(bt.text) { free(bt.text);bt.text = 0; }
}

void aGfxButton::setDefaultSettings(aGfxImage *img,aGfxFont *f,uint32_t data[36]) {
	_s = (_settings){
		img,f,
		{{
...
void aGfxButton::setText(const char *text) {
	if(!text || !*text) return;
...
void aGfxButton::setFont(aGfxFont *font) {
	bt.font = font;
	bt.tw = bt.text? bt.font->stringWidth(bt.text) : 0;
	setTextAlign(bt.align);
}

void aGfxButton::setTextAlign(int align) {
	bt.align = align;
...
void aGfxButton::setInsets(int l,int t,int r,int b) {
	bt.ins = (SDL_Rect){ l,t,r,b };
	setTextAlign(bt.align);
}

void aGfxButton::paint(time_t time) {
	if(isOpaque() && _s.img) {
...
bool aGfxButton::mouseUp(aGfxMouseEvent &me) {
printf("aGfxButton(%p)::mouseUp(source=%p,mouseListener=%p,actionListener=%p)\n",this,me.source,mouseListener,actionListener);
