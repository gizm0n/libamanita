...
aGfxTabset::aGfxTabset(int id,int x,int y,int st) : aGfxComponent(x,y,0,0),tabs(0) {
	setID(id);
	setStyle(st);
	ntabs = 0,act = 0;
	ae = (aGfxActionEvent){ this,0 };
	setMouseListener(this);
}

aGfxTabset::~aGfxTabset() {
	deleteTabs();
}

void aGfxTabset::setDefaultSettings(aGfxImage *img,aGfxFont *f1,aGfxFont *f2,Uint32 data[26]) {
	_s = (_settings){
		img,f1,f2,
		{
...
void aGfxTabset::setStyle(int st) {
	style = st;
	t = style&TABSET_BOTTOM? &_s.bottom : &_s.top;
}

void aGfxTabset::setTabs(const char *str[],int len) {
	if(!str || !len) return;
	deleteTabs();
	act = 0;
	ntabs = len;
...
void aGfxTabset::setActiveTab(int a) {
...
int aGfxTabset::getPreviousTab(int n) {
	if(tabs) {
...
bool aGfxTabset::mouseDown(aGfxMouseEvent &me) {
...
void aGfxTabset::paint(time_t time) {
	if(!tabs) return;
	SDL_Rect *r;
	int i,x = 0,w;
	for(i=0; i<ntabs; i++) {
