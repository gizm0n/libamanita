...
void aGfxGraphics::initSDL(const char *caption,const char *ic1,SDL_Surface *ic2,int w,int h,int d,int f) {
	if(ic1 || ic2) {
		if(ic1) {
			ic2 = SDL_LoadBMP(ic1);
...
void aGfxGraphics::pixel(int x,int y,uint32_t c) {
	int bpp = canvas->format->BytesPerPixel;
	char *p = ((char *)canvas->pixels)+x*bpp+y*canvas->pitch,*cp = (char *)&c;
	for(; bpp; bpp--) *p++ = *cp++;
}

void aGfxGraphics::draw(int x,int y,SDL_Surface *s,SDL_Rect *src) {
	SDL_Rect dst = { x,y,0,0 };
	SDL_BlitSurface(s,src,canvas,&dst);
}

// Bresenhem's line algorithm
void aGfxGraphics::drawLine(int x1,int y1,int x2,int y2,uint32_t c) {
...
void aGfxGraphics::drawDottedRect(int x,int y,int w,int h,uint32_t *p) {
...
void aGfxGraphics::fillRect(int x,int y,int w,int h,uint32_t c) {
	SDL_Rect r = (SDL_Rect){x,y,w,h};
	SDL_FillRect(canvas,&r,c);
}


bool aGfxGraphics::screenDump(const char *fn) {
	return aGfxImage::save(fn,screen);
}
