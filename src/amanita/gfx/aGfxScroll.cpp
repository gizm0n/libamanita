...
#include <amanita/gfx/aGfxScroll.h>
...
aGfxScroll::aGfxScroll(int id,int x,int y,int w,int h,int st) : aGfxComponent(x,y,w,h) {
	...
	add(bmin);
	bmax = new aGfxIconButton(1);
	bmax->setActionListener(this);
	add(bmax);
	bslide = new aGfxIconButton(2);
...
aGfxScroll::~aGfxScroll() {}

void aGfxScroll::setDefaultSettings(aGfxImage *img,uint32_t data[22]) {
	_s = (_settings){
		img,
...
void aGfxScroll::setStyle(int st) {
	style = st;
...
bool aGfxScroll::mouseDrag(aGfxMouseMotionEvent &mme) {
...
bool aGfxScroll::actionPerformed(aGfxActionEvent &ae) {
...
void aGfxScroll::paint(time_t time) {
