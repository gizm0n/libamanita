...
aGfxText::aGfxText(int w,int h,const char *str) {
...
void aGfxText::resizeText(int n) {
	while(drawing);
	if(n<0xff) n = 0xff;
...
void aGfxText::resizeLines(int n) {
	while(drawing);
	if(n<0xff) n = 0xff;
...
void aGfxText::leftShift(int n,int len) {
...
void aGfxText::rightShift(int n,int len) {
	if(!len) return;
...
void aGfxText::insert(char ch,int n) {
fprintf(stderr,"aGfxText::insert('%c',%d)\n",ch,n);
fflush(stderr);
...
void aGfxText::insert(const char *str,int n) {
	if(!str || !*str) return;
	if(n>=(int)textLen) n = textLen;
	int l = strlen(str),i;
...
void aGfxText::setText(const char *str) {
	while(drawing);
	char *t = text;
	if(textLines) delete[] textLines;
	text = 0,textLen = 0,textCap = 0,textLines = 0,textLinesLen = 0,textLinesCap = 0;
...
int aGfxText::findLine(int pos) {
	for(int i=textLinesLen-1; i>=0; i--) if(pos>=(int)textLines[i].pos) return i;
	return 0;
}

void aGfxText::arrange(int from) {
//fprintf(stderr,"aGfxText::arrange(from=%d,textLen=%d,width=%d)\n",from,textLen,width);
//fflush(stderr);
...
//fprintf(stderr,"aGfxText::arrange(line=%d,length=%d,width=%d,pos=%d)\n",l,textLines[l].length,textLines[l].width,p);
//fflush(stderr);
		p -= i-len;
		textLines[++l].pos = p;
	...
	textLinesLen = l;
//fprintf(stderr,"aGfxText::arrange(textLines=%d)\n",textLinesLen);
//fflush(stderr);
}

void aGfxText::drawText(int x,int y,int a,bool showCaret) {
	if(!font) return;
	drawing = true;
	int i = 0;
	const aFontmetrics &metrics = font->getFontMetrics();
