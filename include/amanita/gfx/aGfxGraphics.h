#ifndef _AMANITA_GFX_GRAPHICS_H
#define _AMANITA_GFX_GRAPHICS_H

/**
 * @file amanita/gfx/aGfxGraphics.h  
 * @author Per Löwgren
...
public:
	...
	static void init(const char *caption,aGfxImage *ic,int w,int h,int d,int f) { initSDL(caption,0,ic->getSurface(),w,h,d,f); }
	static void init(const char *caption,aGfxImage &ic,int w,int h,int d,int f) { initSDL(caption,0,ic.getSurface(),w,h,d,f); }
	...
	aGfxGraphics() : screen(0),canvas(0),icon(0),font(0) {}
	aGfxGraphics(aGfxImage *sc) : screen(sc->surface),canvas(sc->surface),icon(0),font(0) {}
	aGfxGraphics(aGfxImage &sc) : screen(sc.surface),canvas(sc.surface),icon(0),font(0) {}
	aGfxGraphics(SDL_Surface *sc) : screen(sc),canvas(sc),icon(0),font(0) {}
	~aGfxGraphics() {}
	...
	void setCanvas(aGfxImage *image) { setCanvas(image->getSurface()); }
	void setCanvas(aGfxImage &image) { setCanvas(image.getSurface()); }
	...
	void draw(int x,int y,aGfxImage *s,int i) { draw(x,y,s->surface,s->getCell(i)); }
	void draw(int x,int y,aGfxImage &s,int i) { draw(x,y,s.surface,s.getCell(i)); }
	void draw(int x,int y,aGfxImage *s,SDL_Rect *src) { draw(x,y,s->surface,src); }
	void draw(int x,int y,aGfxImage &s,SDL_Rect *src) { draw(x,y,s.surface,src); }
	void draw(int x,int y,aGfxImage *s,SDL_Rect &src) { draw(x,y,s->surface,&src); }
	void draw(int x,int y,aGfxImage &s,SDL_Rect &src) { draw(x,y,s.surface,&src); }
...
extern aGfxGraphics g;


#endif /* _AMANITA_GFX_GRAPHICS_H */
