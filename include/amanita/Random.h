 ...
 * 
 * How to use the class is extremely simple. There is a global instance of Random that may
 * be used anywhere where amanita/Random.h is included. It is named "rnd". You may also
 * instantiate Random as an object to keep track of the number sequence, and to make certain
 * it's not affected somewhere else in the program, because this could easily happen with the
 * global rnd.
 * 
 * Instantiating Random has it advantages in games for example, wher you perhaps want to ensure
 * that a certain sequence of random events always happens within a specific game, to avoid that
 * the player saves and loads and saves and loads to gain advantages. By storing Random with the
...
/** @var rnd The global instance of Random.
 * To use rnd, all you need to do is include amanita/Random.h and link to libamanita.*/
extern Random rnd;
